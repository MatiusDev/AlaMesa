const RestaurantCard = (restaurant) => {
  const state = {
    stars: '★'.repeat(Math.round(restaurant.rating || 0)).padEnd(5, '☆'),
    img: restaurant.image || `https://picsum.photos/seed/${encodeURIComponent(restaurant.name || 'restaurant')}/800/600`
  }
  ...
  return `
    <article class="group overflow-hidden rounded-[var(--am-radius)] border border-neutral-200 bg-white shadow-soft hover:shadow-lg transition">
      <div class="relative aspect-[4/3] overflow-hidden">
        <img src="${state.img}" alt="${restaurant.name}" class="h-full w-full object-cover group-hover:scale-105 transition" />
      ...
      <div class="p-4">
        ...
        <div class="mt-2 text-sm">
          <span class="text-am-600">${state.stars}</span>
